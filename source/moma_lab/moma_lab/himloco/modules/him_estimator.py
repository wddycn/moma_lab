...
class HIMEstimator(nn.Module):
    ...
    def update(self, obs_history, next_critic_obs, lr=None):
        if not torch.isfinite(obs_history).all():
            raise ValueError("NaN/Inf detected in HIMEstimator observation history")
        if not torch.isfinite(next_critic_obs).all():
            raise ValueError("NaN/Inf detected in HIMEstimator critic targets")
        ...
        # Isaac Lab applies a term's scale after clipping. The largest configured
        # scale here is 2.0 for base linear velocity, hence the 200 bound.
        if vel.abs().max() > 200.001 or next_obs.abs().max() > 200.001:
            raise ValueError("HIMLoco observation clipping contract was violated")
